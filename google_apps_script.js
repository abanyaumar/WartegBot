...
function getSummary(restaurant, days, startDateParam, periodTag) {
  ...
  function getTargetRows(data) {
    ...
    // Deduplicate: if a date appears more than once (re-entry/correction),
    // keep only the LAST row for that date (most recent entry = correction).
    var seen = {};
    var deduped = [];
    for (var i = filtered.length - 1; i >= 0; i--) {
      var ds = filtered[i][0] ? toDateStr(filtered[i][0]) : "";
      if (ds && !seen[ds]) { seen[ds] = true; deduped.unshift(filtered[i]); }
    }
    return deduped;
  }
