...
def extract_and_audit(image_bytes, restaurant):
    ...
    resp = client.models.generate_content(model="gemini-1.5-flash-latest", contents=[prompt, img_data])
...
def analyze_belanja_detail(images_bytes_list, restaurant, main_data):
    ...
    try:
        resp = client.models.generate_content(model="gemini-1.5-flash-latest", contents=[prompt] + parts)
...
def extract_pengeluaran(content_data, restaurant, is_image=False):
    ...
    try:
        if is_image:
            ...
            resp = client.models.generate_content(model="gemini-1.5-flash-latest", contents=[prompt, img])
        else:
